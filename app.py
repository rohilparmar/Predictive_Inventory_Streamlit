...
# 2. Ultra-Modern CSS with Animation
st.markdown("""
    <style>
    ...
    /* Notification Boxes with Pulse effect */
    .alert-box {
...
def get_real_google_trends(keyword="New Fashion"):
    try:
        pytrends = TrendReq(hl='en-IN', tz=330)
        pytrends.build_payload([keyword], cat=18, timeframe='now 7-d', geo='IN')
        data = pytrends.interest_over_time()
        if not data.empty:
            change = data[keyword].iloc[-1] - data[keyword].iloc[-7]
            return change
    except:
        return None
    return None
...
def get_market_alerts():
    ...
    # 1. Seasonality: Summer Peak (Current)
    if today.month in [3, 4, 5, 6]:
        alerts.append({"type": "info", "msg": "☀️ **SUMMER PEAK:** Ahmedabad humidity levels rising. Market shift towards light linens and cottons."})
    
    # 2. Real Google Trends Integration
    # Track "Ethnic Wear" for shop relevancy
    trend_change = get_real_google_trends("Ethnic Wear")
    if trend_change is not None and trend_change > 0:
        alerts.append({"type": "high", "msg": f"🔥 **HOT TREND:** Search for 'Ethnic Wear' is up by {int(trend_change)} pts. High purchase intent detected!"})
    else:
        alerts.append({"type": "info", "msg": "🔍 **MARKET INSIGHT:** Steady demand for 'Casual Basics'. Inventory health is stable."})

    # 3. Festival Prep
    diwali_date = datetime.datetime(today.year, 11, 1)
    days_to_diwali = (diwali_date - today).days
    alerts.append({"type": "info", "msg": f"📅 **STRATEGY:** {days_to_diwali} days to Diwali. Recommended: Start cataloging for Winter-Festive stock."})
...
if page == "🏠 Core Dashboard":
    ...
    with m1: st.markdown('<div class="glass-card"><p class="metric-label">Total Stores</p><p class="metric-val">54</p></div>', unsafe_allow_html=True)
    with m2: st.markdown('<div class="glass-card"><p class="metric-label">Stock Health</p><p class="metric-val">94.8%</p></div>', unsafe_allow_html=True)
    with m3: st.markdown('<div class="glass-card"><p class="metric-label">Predictions/Day</p><p class="metric-val">1.2k</p></div>', unsafe_allow_html=True)
    with m4: st.markdown('<div class="glass-card"><p class="metric-label">System Uptime</p><p class="metric-val">99.9%</p></div>', unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if st.button("🚀 INITIATE PREDICTION"):
            with st.spinner("Analyzing Market Trends..."):
                ...
                except: st.error("Model File 'inventory_model.joblib' not found in repository.")
    ...
    with col_r:
        if 'current_pred' in st.session_state:
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge = {'axis': {'range': [None, p*2]}, 'bar': {'color': "#38bdf8"}}
            ))
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict(color="white"), height=250)
